...
packages_needed <- c("survival","dplyr","ggplot2","survminer","cowplot","gridExtra","pROC","swimplot","tidyverse","reshape2","survcomp","epiR","eulerr","data.table")
...
####
Figure_2_top_plot  <- ggarrange(plotlist=list(P1_KM$plot, P2_KM$plot),
                                ncol = 2, nrow = 1, labels = c("A","B"))

Figure_2_top_risk <- ggarrange(plotlist=list(P1_KM$table, P2_KM$table),
                               ncol = 2, nrow = 1)

Figure_2_top_title <- ggdraw() + draw_label(paste0("Fragmentation"), fontface='bold')

Figure_2_top_panel <- plot_grid(Figure_2_top_title, Figure_2_top_plot, Figure_2_top_risk,
                                ncol=1,
...
Figure_2_mid_plot  <- ggarrange(plotlist=list(P1_mut_KM$plot,P2_mut_KM$plot),
                                ncol = 2, nrow = 1, labels = c("C","D"))

Figure_2_mid_risk <- ggarrange(plotlist=list(P1_mut_KM$table,P2_mut_KM$table),
                               ncol = 2, nrow = 1)

Figure_2_mid_title <- ggdraw() + draw_label(paste0("Mutation"), fontface='bold')

Figure_2_mid_panel <- plot_grid(Figure_2_mid_title, Figure_2_mid_plot, Figure_2_mid_risk,
                                ncol=1,
                                rel_heights=c(0.1, 1, 0.5))
...
Figure_2_bottom_plot  <- ggarrange(plotlist=list(P1_ctDNA_KM$plot,P2_ctDNA_KM$plot),
                                   ncol = 2, nrow = 1, labels = c("E","F"))

Figure_2_bottom_risk <- ggarrange(plotlist=list(P1_ctDNA_KM$table,P2_ctDNA_KM$table),
                                  ncol = 2, nrow = 1)

Figure_2_bottom_title <- ggdraw() + draw_label(paste0("Fragmentation + Mutation"), fontface='bold')

Figure_2_bottom_panel <- plot_grid(Figure_2_bottom_title, Figure_2_bottom_plot, Figure_2_bottom_risk,
                                   ncol=1,
                                   rel_heights=c(0.1, 1, 0.5))


ggsave(filename = "Figure_2.pdf",
       plot_grid(Figure_2_top_panel, Figure_2_mid_panel, Figure_2_bottom_panel, nrow = 3),
...
######## swimmer plot
fig3_swimmer_plot <- function(x, y,z) {
...
figure_3_panel_A <- fig3_swimmer_plot(tmp[tmp$Patient_ID %in% P1_model[P1_model$PD_event == 1,]$`Patient ID`,], 
                                      MRD_followup[MRD_followup$Patient_ID %in% P1_model[P1_model$PD_event == 1,]$`Patient ID`,],
                                      "top")

figure_3_panel_B <- fig3_swimmer_plot(tmp[tmp$Patient_ID %in% P1_model[P1_model$PD_event == 0,]$`Patient ID`,],
                                      MRD_followup[MRD_followup$Patient_ID %in% P1_model[P1_model$PD_event == 0,]$`Patient ID`,],
                                      "NA")

figure_3_swimmer <- plot_grid(figure_3_panel_A, figure_3_panel_B, ncol = 1, rel_heights = c(1,2), labels = c("A","B"))

ggsave(filename = "Figure_3.pdf",
       figure_3_swimmer,dpi = 300,width = 8,height = 14)



##### Figure_4

##### Figure 4, sens/spec bar plot + 3 Venn diagram
...
Figure_3_bottom_plot <- ggarrange(plotlist=list(P1_venn_plot, P2_venn_plot
                                                # , AP_venn_plot
),
ncol = 2, nrow = 1, labels = c("A","B"
                               # , "C"
...
ggsave(filename = "Figure_4.pdf",
       plot_grid(Figure_3_bottom_plot, sens_plot, ncol=1, rel_heights=c(0.75, 1), labels = c("","C")),
...
##### Figure S1
###### Figure S1 panel A ROC, panel B boxplot

tmp_P1 <- P1_model
tmp_P1$Group = "Progressed"
tmp_P1[tmp_P1$PD_event == 0,]$Group = "Not progressed"

figure_S1_panel_C <- ggplot(tmp_P1,aes(x=Group,y=Cox_pred, fill = Group)) + geom_boxplot(outlier.shape = NA) +
...
figure_S1_panel_D <- ggplot(tmp_P2,aes(x=Group,y=Cox_pred, fill = Group)) + geom_boxplot(outlier.shape = NA) +
...
figure_S1_panel_A <- ggrocs(rocs = roc.plot.tmp, breaks = seq(0,1,0.2), legendTitel = "LOOCV\n AUC (95% CI)")

ggsave(filename = "Figure_S1.pdf",
       plot_grid(figure_S1_panel_A, figure_S1_panel_C, figure_S1_panel_D, ncol= 3, labels = c("A","B","C"), rel_widths = c(6,4,4)),
...
S2_panel_A <- ggplot(P1_correlation_df, aes(x=maxVAF, y=Cox_pred, 
                                            shape = PD_event,
                                            color = risk
)) + 
  geom_point()+
...
S2_panel_B <- ggplot(P2_correlation_df, aes(x=maxVAF, y=Cox_pred, 
                                            shape = PD_event,
                                            color = risk
)) + 
  geom_point()+
...
P1_plot_S2 <- P1_feature_ratio[which(P1_feature_ratio$variable %like% "shortA" ),]
P1_plot_S2 <- P1_plot_S2[rowSums(is.na(P1_plot_S2)) == 0,]
P1_plot_S2$arm <- factor(P1_plot_S2$arm,
                         levels = c("1p","1q","2p","2q","3p","3q","4p","4q","5p","5q","6p","6q","7p","7q","8p","8q","9p","9q",
                                    ...
                                    "20p","20q","21q","22q"))
P1_plot_S2$value <- 100 * P1_plot_S2$value

P1_plot_S2_new <- unique(P1_plot_S2) %>%
...
ggsave(filename = "Figure_S2.pdf",
       plot_grid(plot_grid(S2_panel_A, S2_panel_B, ncol = 2, nrow = 1,labels = c("A","B")), S2_panel_C, nrow = 2, labels = c("","C")),
...
sens_plot_S2 <- ggplot(data=stats_df[stats_df$label != "Combined timepoints",], aes(x=label, y=Value,fill=type)) +
  geom_bar(colour="black",stat="identity", position = position_dodge(0.7), width = 0.5)+
  geom_errorbar(aes(ymin=Lower, ymax=Upper), width=.2,position = position_dodge(0.7)) + 
  ylab("Sensitivity/Sepcificity") + xlab("Postsurgical") +
  facet_wrap(~ Group,ncol=3) + ylim(0,1) + 
...
ggsave(filename = "Figure_S3.pdf",
       plot_grid(combined_venn_plot, sens_plot_S2, nrow = 1, ncol = 2, rel_widths = c(1,2), scale = c(1,0.9), labels = c("A","B")),
